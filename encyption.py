...
passwordDictionary = {'a': 'dmn|', 'b': 'z4>p', 'c': 'D--j', 'd': '#~YW', 'e': 'Nw?m', 'f': 'QaOa', 'g': 'nhhz', 'h': '1Peq', 'i': '&1\\g', 'j': '10*`', 'k': '|u?&',
...
invertedDictionary = {key: val for val, key in passwordDictionary.items()}
...
def encrypt():
    ...
    print("Code : ", end="")
    for x in password:
        print(passwordDictionary[x], end="")
...
def decrypt():
    ...
    try:
        print("Password : ", end="")
        for x in code:
            print(invertedDictionary[x], end="")
    except Exception:
        print("Invalid code!!")
